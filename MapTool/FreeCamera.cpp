...
_bool CFreeCamera::m_bFixed = false;
...
void CFreeCamera::Priority_Update(_float fTimeDelta)
{
	...
	if (m_pGameInstance->Get_DIKeyState(DIK_W) & 0x80)
		m_pTransformCom->Go_Straight(fTimeDelta * addSpeed);
	if (m_pGameInstance->Get_DIKeyState(DIK_S) & 0x80)
		m_pTransformCom->Go_Backward(fTimeDelta * addSpeed);
	if (m_pGameInstance->Get_DIKeyState(DIK_A) & 0x80)
		m_pTransformCom->Go_Left(fTimeDelta * addSpeed);
	if (m_pGameInstance->Get_DIKeyState(DIK_D) & 0x80)
		m_pTransformCom->Go_Right(fTimeDelta * addSpeed);
	if (m_pGameInstance->Get_DIKeyState(DIKEYBOARD_LCONTROL))
		m_pTransformCom->Go_Down(fTimeDelta * addSpeed);
	if (m_pGameInstance->Get_DIKeyState(DIKEYBOARD_SPACE))
		m_pTransformCom->Go_Up(fTimeDelta * addSpeed);

	_long		MouseMove = { 0 };
	if (m_pGameInstance->Get_RealDIMouseStateDown(MOUSEKEYSTATE::DIMK_RBUTTON)) {
		m_bFixed = !m_bFixed;
	}
	if(!m_bFixed)
	{
