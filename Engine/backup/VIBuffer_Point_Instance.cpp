...
#pragma region INSTANCE_BUFFER
	...
	for (size_t i = 0; i < m_iNumInstance; i++)
	{
		...
		pInstanceVertices[i].vLifeTime = _float2(10.f, 0.f);//m_pGameInstance->Get_Random(m_vLifeTime.x, m_vLifeTime.y), 0.0f);
	}
...
#pragma endregion
	auto* desc = static_cast<POINTINSTANCE_DESC*>(pArg);
...
void CVIBuffer_Point_Instance::Screw(_float fTimeDelta)
{
	_float fRadius = XMConvertToRadians(15.f);
	_float fPerLookDistance = 0.3f;
	_vector vLook = m_pOwner->GetTransformCom()->Get_State(CTransform::STATE_LOOK);
	_vector vStartPos = m_pOwner->GetTransformCom()->Get_State(CTransform::STATE_POSITION) + m_pOwner->GetTransformCom()->Get_State(CTransform::STATE_UP)
		+ vLook * m_fLookOffset + XMVectorSet(0.f, fRadius, 0.f, 0.f);
	D3D11_MAPPED_SUBRESOURCE SubResource = {};
	...
	for (size_t i = 0; i < m_iNumInstance; i++)
	{
		_vector vPerPos = vStartPos + vLook * fPerLookDistance * (_float)i;
		//vPerPos = XMVector3TransformCoord(vPerPos, XMMatrixRotationAxis(vLook, (fRadius * (_float)i)));
		XMStoreFloat4(&pVertices[i].vTranslation, vPerPos);
	}
